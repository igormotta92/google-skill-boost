...
Analyzing Natality Data Using Agent Platform and BigQuery
...
Task 1. Review the basics of SQL
...
Task 2. Explore the BigQuery console
...
Upload queryable data
In this section you pull in some public data into your project so you can practice running SQL commands in BigQuery.

In the BigQuery, click on the Explorer icon (first icon in the top menu bar for BigQuery), click + Add data.
BigQuery Explorer page with green rectangle around the +Add Data button

Click Star a project by name.

Type the project name: bigquery-public-data

Click Star.

Click on the back arrow next to Add data to return to the Explorer pane.

It's important to note that you are still working out of your lab project in this new tab. All you did was pull a publicly accessible project that contains datasets and tables into BigQuery for analysis — you didn't switch over to that project. All of your jobs and services are still tied to your Google Skills account.

In the Explorer pane, click on the bigquery-public-data project name.

Click Datasets.

You have access to many datasets in the bigquery-public-data project including london_bicycles.

At the bottom right of the Datasets pane, click on the forward arrow to navigation through the dataset names, and then click on the dataset named london_bicycles.
There are two tables in the london_bicycles dataset:

cycle_hire
cycle_stations
Click on the cycle_hire table, and then click the Preview tab to preview the data columns and values.
...
Use SELECT, FROM, and WHERE in BigQuery
...
In the Untitled query tab, copy and paste the following query:
...
This initiates a download, which saves this query as a CSV file. Note the location and the name of this downloaded file — you will need it soon.
...
This initiates a download, which saves this query as a CSV file. Note the location and the name of this downloaded file — you will need it in the following section.
...
Enter a unique name for your bucket (such as your project ID Project ID), keep all other settings as default, and click Create.
...
On the Navigation menu (Navigation menu icon), click Cloud SQL.

On the Instances page, under Get Started for the MySQL tab, click Create Development Instance.
...
For Database version, select MySQL 8.0. Leave the default value for Minor version.

For Instance ID, type: my-demo

For Password, type: ChangeMe1!

For Zonal availability, select Multiple zones (Highly available).

Expand the drop-down menu for Specify zones, and select <Lab Zone> as the Primary Zone. Leave the default value for Secondary zone.
...
When prompted, click Continue, and then click Authorize.

Run the pre-populated command to connect to your SQL instance, replacing my-demo if you used a different name for your instance:

gcloud sql connect my-demo --user=root --quiet
Copied!
Note: It may take a minute to connect to your instance. If you get a message that "Operation failed because another operation was already in progress", you need to wait for the SQL instance to finish being created, and then try to connect again.
When prompted, type the root password you set for the instance: ChangeMe1!
Note: The cursor will not move.
...
Leave Cloud Shell open, and proceed to the next task.
...
Repeat steps 1-5 to import the other CSV file named end_station_data.csv into the bike database table named london2.
...
Run the following command to make sure that london2 has also been populated:
SELECT * FROM london2;
Copied!
